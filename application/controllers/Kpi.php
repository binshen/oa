...
//		$this->load->model('kpi_model');
	...
	public function list_kpi($page=1){
		$data = $this->kpi_model->list_kpi($page);
		$base_url = "/kpi/list_kpi";
		$pager = $this->pagination->getPageLink($base_url, $data['total'], $data['limit']);
		$this->assign('pager', $pager);
		$this->assign('data', $data);
		$this->show('kpi/list_kpi');
	}
	
	public function add_kpi_month(){
//		$user_dept = $this->task_model->get_users_dept();
//		$this->assign('user_dept', $user_dept);
		$this->show('kpi/add_kpi_month');
	}
//
