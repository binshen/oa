...
//     	$data['items'] = $this->db->get()->result_array();
	...
	//会议列表
	public function list_meeting($page)
	{
		$data['limit'] = $this->limit;
		//获取总记录数
		$this->db->select('count(1) num')->from('meeting_main');
		if($this->input->post('title')){
			$this->db->like('title',$this->input->post('title'));
		}
		$num = $this->db->get()->row();
		...
		//获取详细列
		$this->db->select('a.id,title,a.cdate,b.rel_name')->from('meeting_main a');
		$this->db->join('users b','a.from_uid=b.id','left');
		if($this->input->post('title')){
			$this->db->like('title',$this->input->post('title'));
			$data['title'] = $this->input->post('title');
	...
	public function save_meeting(){
		...
		$this->db->insert('meeting_main',$data);

		$id = $this->db->insert_id();

		foreach ($this->input->post('uid') as $v){
			$this->db->insert('meeting_list',array('mid'=>$id,'uid'=>$v,'read'=>0));
		}
	...
	public function get_meeting($id){
		$data = $this->db->select('a.*,b.rel_name')->from('meeting_main a')
			->join('users b','a.from_uid=b.id','left')
			->where('a.id',$id)->get()->row_array();
		return $data;
	}
